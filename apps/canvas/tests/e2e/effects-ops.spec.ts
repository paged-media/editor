...
const EFFECT_CASES: EffectCase[] = [
  ...
  },
  {
    ...
    // WAS a render gap — the published engine, not the test.
    //
    // The model write landed and read back while the pixels did not
    // move under canvas-wasm 0.62.0 (tagged 2026-08-22), which
    // approximated a directional feather by stamping a WHITE
    // blurred path inside the object's own layer with SrcOver. That
    // paints ink where InDesign REMOVES it, so on this fixture's ground
    // it was invisible — and on a tinted one a plainly wrong halo,
    // which is how the annual's page 59 lost the panel under the cloud.
    //
    // core 72cd7b3 (2026-09-06) replaced it with the real coverage mask
    // the PDF lane uses, shared across all three lanes, and the pin now
    // carries it (0.63.0). Render gate ENFORCED — the flag is gone, the
    // same way the other six effects lost theirs.
  },
];
