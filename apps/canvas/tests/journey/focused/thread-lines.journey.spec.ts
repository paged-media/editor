...
test.describe("journey · text thread overlay", () => {
  test("threads draw on toggle, name their frames, and clear again @feat:editor-tools.overlays @level:gesture", async ({
    page,
  }) => {
  ...
  test("an unthreaded document draws nothing with the overlay ON @feat:editor-tools.overlays", async ({
    page,
  }) => {
